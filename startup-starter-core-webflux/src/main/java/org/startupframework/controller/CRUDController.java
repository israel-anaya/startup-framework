...
public interface CRUDController<DTO extends DataTransferObject> {
	...
	@GetMapping()
	@ResponseBody
	Flux<DTO> getAllItems();

	@GetMapping("/{id}")
	@ResponseBody
	Mono<ResponseEntity<DTO>> getItem(@PathVariable("id") String id);

	@PostMapping()
	@ResponseBody
	Mono<ResponseEntity<DTO>> createItem(@RequestBody DTO item);

	@PutMapping()
	@ResponseBody
	Mono<ResponseEntity<DTO>> updateItem(@RequestBody DTO item);
	
	@DeleteMapping("/{id}")
	@ResponseBody
	Mono<ResponseEntity<DTO>> deleteItem(@PathVariable("id") String id);
